import { getDependenciesChanges, getManageDependenciesCommand } from '.';
...
test('Should create add dependencies command correctly', () => {
  ...
  const command = getManageDependenciesCommand(mockParams, 'add');

  expect(command).toEqual(
    'yarn add react-native-location@^2.5.0 react-native-maps@^0.29.4 react-native-pdf@^6.5.0 react-native-rate@^1.2.9 react-native-reanimated@^2.2.3 ',
  );
});
...
test('Should create upgrade dependencies command correctly', () => {
  ...
  const command = getManageDependenciesCommand(mockParams, 'upgrade');

  expect(command).toEqual(
    'yarn upgrade react-native-location@^2.5.0 react-native-maps@^0.29.4 react-native-pdf@^6.5.0 react-native-rate@^1.2.9 react-native-reanimated@^2.2.3 ',
  );
});
...
describe('Testing dependencies changes', () => {
  test('Should not consider diff if source package have same dependency and version as target package', () => {
    ...
    const { added, updated } = getDependenciesChanges(source, target);
